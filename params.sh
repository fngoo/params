...
echo 'for param in `cat 2.txt`; do line=`head -$num /root/script/3_httprobe/$input | tail -1` ; echo "$param" > togrep.txt ; grep -oP "?" togrep.txt > toif.txt ; if [ -s toif.txt ]; then echo "${line}&${param}=1" >> /root/script/3_httprobe/params_xss_test.txt; else echo "${line}?${param}=1" >> /root/script/3_httprobe/params_xss_test.txt; fi; rm togrep.txt ; rm toif.txt ; num=$((num+1)) ; done' >> /root/script/3_httprobe/dir_$i/${i}.sh
