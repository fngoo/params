...
cat /root/script/3_httprobe/exe.sh | parallel --jobs 0 --progress --delay 0.5 --retries 1
...
echo "for c in "\`cat /root/script/3_httprobe/dir_$i/${input}\`"; do echo "$c" >> \${output}/xss.txt ; echo '------- ------------ ------------ -------------' >> \${output}/xss.txt ; python3 xsstrike.py -u \"\$c\" --console-log-level GOOD --skip --blind >> \${output}/xss.txt ; echo;echo;echo;echo;echo; done" >> /root/script/3_httprobe/dir_$i/${i}.sh
